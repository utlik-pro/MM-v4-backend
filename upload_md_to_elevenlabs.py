#!/usr/bin/env python3
"""
Загрузка MD файлов из /quarters в ElevenLabs Knowledge Base
"""
...
def upload_md_file(file_path, api_key):
    """Загружает MD файл в ElevenLabs"""
    ...
    # Загружаем как текстовый файл (.txt формат принимается ElevenLabs)
    files = {
        'file': (f'{name}.txt', content.encode('utf-8'), 'text/plain')
    }
    data = {
        'name': name
    }
    ...
    if response.status_code in [200, 201]:
        ...
        kb_id = result.get('knowledge_base_id', result.get('id', 'N/A'))
...
def main():
    ...
    print("=" * 60)
    print("📤 Загрузка MD файлов в ElevenLabs Knowledge Base")
    print("=" * 60)
    print(f"\n📂 Папка: {quarters_dir}")
    ...
    for i, filename in enumerate(quarter_files, 1):
        ...
        success, kb_id, name = upload_md_file(file_path, api_key)
    ...
    if uploaded_ids:
        with open('/Users/admin/MM-RAG/uploaded_kb_ids.txt', 'w') as f:
            f.write('\n'.join(uploaded_ids))
        print(f"\n✅ ID документов сохранены в uploaded_kb_ids.txt")
        print(f"\n🔧 Теперь запустите:")
        print(f"   python3 add_documents_to_agent.py")
...
if __name__ == "__main__":
    try:
        ids = main()
        print(f"\n✨ Готово! Загружено {len(ids)} документов")
