...
class ElevenLabsAutoSync:
    ...
    def get_all_kb_documents_cached(self, ttl_minutes: int = 60, use_cache_only: bool = False) -> List[Dict]:
        ...
        try:
            ...
            log(f"  ❌ Критическая ошибка получения документов: {e}")
            log(f"  💡 Возвращаем пустой список, продолжаем без удаления старых версий")
            return []
...
<body><pre>{markdown_content}</pre></body>
    ...
    def full_sync(self, files_dir: str = 'quarters', dry_run: bool = False, no_delete: bool = False, changed_files: List[str] = None):
        ...
        """
        print("=" * 70)
        print("🚀 АВТОМАТИЧЕСКАЯ СИНХРОНИЗАЦИЯ ELEVENLABS KNOWLEDGE BASE")
        print("=" * 70)
        ...
        # В инкрементальном режиме используем только свежий кэш (не делаем новый запрос к API)
        # Это быстрее и избегает зависаний при проблемах с API
        if changed_files:
            log("   ⚡ Инкрементальный режим: используем только кэш (TTL: 24 часа, без запроса к API)")
            all_docs = self.get_all_kb_documents_cached(ttl_minutes=60 * 24, use_cache_only=True)
        else:
            # Полная синхронизация: можно обновить кэш
        ...
        if uploaded_ids:
            ...
            ready_ids = self.wait_for_indexing(uploaded_ids, max_wait=120)
