...
Автоматически структурирует JSON данные в Markdown формат
"""
...
class BirDataParser:
    ...
    def generate_index_file(self, output_dir: str = "quarters"):
        ...
        index_content = "# 🏘️ Индекс кварталов недвижимости\n\n"
        ...
        index_content += f"- **Всего объектов:** {total_objects}\n"
    ...
    def analyze_problematic_quarters(self):
        """Анализирует проблемные данные кварталов"""
        if not self.data:
            print("Данные не загружены")
            return
        ...
        for item_id, item in self.data.items():
            quarter_text = item.get('Quarter', '')
            decoded = self.decode_unicode(quarter_text)
            
            # Собираем образцы проблемных кварталов
            if not quarter_text or not decoded.strip():
                house_number = item.get('NumberHouse', '')
                house_name = self.decode_unicode(item.get('NameHouse', ''))
                address = self.decode_unicode(item.get('Address', ''))
                ...
                # Анализируем по домам
                house_key = f"{house_number} ({house_name})"
                if house_key not in house_analysis:
                    house_analysis[house_key] = []
                house_analysis[house_key].append(item_id)
                
                if decoded not in quarter_samples:
                    quarter_samples[decoded] = []
                quarter_samples[decoded].append(item_id)
        
        print(f"\n🔍 Анализ проблемных данных:")
        print(f"Найдено {len(problematic_items)} объектов с проблемными кварталами")
        
        if house_analysis:
            print("\n🏠 Анализ по домам:")
            for house_info, item_ids in house_analysis.items():
                print(f"  {house_info}: {len(item_ids)} объектов")
                # Показываем первые несколько адресов для каждого дома
                for i, item_id in enumerate(item_ids[:3]):
                    item = self.data[item_id]
                    address = self.decode_unicode(item.get('Address', 'N/A'))
                    print(f"    {address}")
                if len(item_ids) > 3:
                    print(f"    ... и еще {len(item_ids) - 3} объектов")
        
        if quarter_samples:
            print("\n📋 Образцы проблемных значений Quarter:")
            for quarter_value, item_ids in quarter_samples.items():
                print(f"  '{quarter_value}' - {len(item_ids)} объектов")
...
def main():
    """Основная функция"""
    print("🏘️ Парсер данных недвижимости BIR.BY")
    print("=" * 50)
    ...
    problematic_items = parser.analyze_problematic_quarters()
